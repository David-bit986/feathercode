...
const files = execSync('findstr /m /s /c:"@hugeicons/core-free-icons" "src\\*.tsx" "src\\*.ts"', {encoding: 'utf8'}).trim().split('\r\n');
...
files.forEach(f => {
  ...
  // Find all <HugeiconsIcon icon={IconName} ... /> and replace
  ...
  // Replace variable usages (like in object dictionaries, or typing)
  ...
  if (usedIcons.length > 0) {
    ...
    const existingLucide = content.match(/import\s+\{([^}]+)\}\s+from\s+["']lucide-react["'];?/);
    if (existingLucide) {
      const existingIcons = existingLucide[1].split(',').map(i => i.trim()).filter(Boolean);
      lucideImports.forEach(i => {
        if (!existingIcons.includes(i)) existingIcons.push(i);
      });
      content = content.replace(existingLucide[0], `import { ${existingIcons.sort().join(', ')} } from "lucide-react";`);
    } else {
...
console.log("Done refactoring missed icons!");
