...
const OWNED_MARKERS: [&str; 2] = ["notify;FeatherCode;", "fc;notify"];

// Gated on FC_TERMINAL; no-op outside FeatherCode. Returns the sequence via
// `terminalSequence` because hooks lost /dev/tty access in v2.1.139.
fn hook_cmd(event: &str) -> String {
    format!(
        r#"[ -n "$FC_TERMINAL" ] && printf '{{"terminalSequence":"\\u001b]777;notify;FeatherCode;{event}\\u0007"}}' || true"#
    )
}
...
pub fn agent_enable_claude_hooks() -> Result<(), String> {
    ...
    let tmp = path.with_extension("json.fc-tmp");
...
pub fn agent_claude_hooks_status() -> bool {
    ...
    HOOK_EVENTS
        .iter()
        .all(|(_, m)| content.contains(&format!("notify;FeatherCode;{m}")))
}
...
mod tests {
    ...
    fn adds_all_event_hooks_to_empty_config() {
        ...
        assert!(command(&out, "Notification", 0).contains("notify;FeatherCode;attention"));
        assert!(command(&out, "Stop", 0).contains("notify;FeatherCode;finished"));
        assert!(command(&out, "UserPromptSubmit", 0).contains("notify;FeatherCode;working"));
        ...
        let legacy = json!({
            "hooks": {
                "Notification": [
                    { "hooks": [ {
                        "type": "command",
                        "command": "[ -n \"$FC_TERMINAL\" ] && printf '\\033]777;fc;notify\\033\\\\' > /dev/tty || true"
                    } ] }
                ]
            }
        });
