...
if [ -z "$__FC_HOOKS_LOADED" ]; then
  ...
  _fc_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_fc_urlencode "$PWD")"
    if [ -z "$__FC_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __FC_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_fc_precmd:"*) ;;
    *) PROMPT_COMMAND="_fc_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
