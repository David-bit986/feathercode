...
# zsh, so we shadow $? into `_fc_ret`.

{
  _fc_user_zdotdir="${FC_USER_ZDOTDIR:-$HOME}"
  [ -f "$_fc_user_zdotdir/.zshrc" ] && source "$_fc_user_zdotdir/.zshrc"
...
if [[ -z "$__FC_HOOKS_LOADED" ]]; then
  ...
  _fc_precmd() {
    local _fc_ret=$?
    printf '\e]133;D;%s\e\\' "$_fc_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_fc_urlencode "$PWD")"
  ...
  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _fc_precmd
    add-zsh-hook preexec _fc_preexec
  fi
