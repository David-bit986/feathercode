...
import { useSessionsStore, chats, seedMessages, touchChat } from "../store/sessionsStore";
import {
...
export type AgentRuntime = {
  getOrCreateChat: (sessionId: string) => Chat<UIMessage>;
...
function makeChat(sessionId: string): Chat<UIMessage> {
  ...
  const transport = createContextAwareTransport({
    ...
    getSkillsInstructions: () => {
      ...
      return getSkillsInstructions(skills);
    },
    ...
    onStep: (step) => {
      useAgentMetaStore.getState().patchAgentMeta({ step });
    },
    onCompact: (info) => {
      useAgentMetaStore.getState().patchAgentMeta({
        compactionNotice: { droppedCount: info.droppedCount, at: Date.now() },
      });
    },
    onFinishMeta: (info) => {
      useAgentMetaStore.getState().patchAgentMeta({ hitStepCap: info.hitStepCap });
    },
    onUsage: (delta) => {
      const cur = useAgentMetaStore.getState().agentMeta.tokens;
      useAgentMetaStore.getState().patchAgentMeta({
        tokens: {
...
export async function sendMessage(text: string): Promise<boolean> {
  ...
  if (
    providerNeedsKey(getModel(chatState.selectedModelId as ModelId).provider) &&
    !getActiveProviderKey()
  )
    return false;
