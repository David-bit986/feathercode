...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S feathercode-bin";
    case "debian":
      return `sudo apt install ./FeatherCode_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./FeatherCode-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `FeatherCode v${manual.version} is available`
                  : `FeatherCode v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart FeatherCode to finish installing."
              : downloading
                ? progress !== null
